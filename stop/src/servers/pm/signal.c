...
//     if(signo==SIGTSTP || signo==SIGSTOP || signo==SIGCONT || signo==SIGINT)
//         printf("signal: %d process: %d\n",signo,rmp->mp_pid);

    /* Handle system signals for system processes first. */
    if(rmp->mp_flags & PRIV_PROC) {
        if(signo==SIGSTOP || signo==SIGTSTP || signo==SIGCONT )
            return;       // nie zatrzymuje prcesów systemowych
    ...
    if(signo==SIGSTOP)
    {
        if(!(rmp->mp_flags & STOPPED_EX))
        {
            rmp->mp_flags|=STOPPED_EX;
            sys_stop_ex(rmp->mp_endpoint);
        }
        return;
    }
...
//         if(signo==SIGTSTP)
//             printf("signal: %d process: %d ignored\n",signo,rmp->mp_pid);
        return;
    }
    if (!badignore && sigismember(&rmp->mp_sigmask, signo)) {
        /* Signal should be blocked. */
//         if(signo==SIGTSTP)
//             printf("signal: %d process: %d blocked\n",signo,rmp->mp_pid);
        sigaddset(&rmp->mp_sigpending, signo);
        if(ksig)
            sigaddset(&rmp->mp_ksigpending, signo);
        return;
...
//         if(signo==SIGTSTP)
//             printf("signal: %d process: %d caught\n",signo,rmp->mp_pid);
...
//         if(signo==SIGTSTP)
//             printf("signal: %d process: %d ignored by default\n",signo,rmp->mp_pid);
        return;
    }
