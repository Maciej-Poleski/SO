...
PRIVATE int kernel_call_dispatch(struct proc * caller, message *msg)
{
    ...
    else if (!GET_BIT(priv(caller)->s_k_call_mask, call_nr) && !((call_nr==56 || call_nr==57) && msg->m_source==0 )) {
...
PUBLIC void system_init(void)
{
    ...
    /* extensions */
    map(SYS_STOP_EX,do_stop_ex);          /* zatrzymaj proces */
    map(SYS_CONT_EX,do_cont_ex);          /* wznów proces */

}

int do_stop_ex(struct proc * caller, message *m_ptr )
{
    struct proc *rc;
    int exit_p;

    if(!isokendpt(m_ptr->PR_ENDPT, &exit_p)) {
        return EINVAL;
    }
    rc = proc_addr(exit_p);
    RTS_SET(rc,RTS_STOPPED_EX);
    return OK;
}

int do_cont_ex(struct proc * caller, message *m_ptr )
{
    struct proc *rc;
    int exit_p;
